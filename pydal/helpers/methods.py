    ...
    for item in text.split():
        if field is None:
            ...
            if op == "=":
                ...
                if op == "contains":
                    new_query = field.cast("text").contains(value)
                else:
...
def auto_represent(field):
    ...
    if (
        ...
        and hasattr(field, 'referent')
        and field.referent is not None
    ):
        return _repr_ref(field.referent.table)
    if (
        field.db
        ...
        return _repr_ref_list(field.referent.table)
    if (
        field.db
        and field.type.startswith("reference")
